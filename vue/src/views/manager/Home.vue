    ...
    export default {
        ...
        data() {
            return {
                ...
                notices: [],
                pieLostOptions: { ...pieOptions },  // 复制一份 pieOptions
                pieFoundOptions: { ...pieOptions },  // 复制一份 pieOptions
            };
        },
        created() {
            ...
            loadLost() {
                this.$request.get('/echarts/lost').then(res => {
                    if (res.code === '200') {
                        ...
                        let myChart = echarts.init(chartDom);
                        this.pieLostOptions.title.text = res.data.text;
                        this.pieLostOptions.title.subtext = res.data.subText;
                        this.pieLostOptions.series[0].name = res.data.name;
                        this.pieLostOptions.series[0].data = res.data.data;
                        myChart.setOption(this.pieLostOptions);
                    } else {
                        this.$message.error(res.msg);
                    }
                });
            ...
            loadFound() {
                this.$request.get('/echarts/found').then(res => {
                    if (res.code === '200') {
                        ...
                        let myChart = echarts.init(chartDom);
                        this.pieFoundOptions.title.text = res.data.text;
                        this.pieFoundOptions.title.subtext = res.data.subText;
                        this.pieFoundOptions.series[0].name = res.data.name;
                        this.pieFoundOptions.series[0].data = res.data.data;
                        myChart.setOption(this.pieFoundOptions);
                    } else {
                        this.$message.error(res.msg);
                    }
                });
